      ...
      <div className="flex items-center gap-1.5 md:gap-2 shrink-0">
        {canInstall && (
          <Button
            ...
            disabled={installing}
            className="gradient-warm text-primary-foreground gap-1.5 h-10 px-2.5 md:px-3 text-xs"
            title={t('install_app')}
          >
        ...
        <Button variant="outline" size="sm" onClick={toggleLang} className="gap-1 font-semibold text-xs md:text-sm px-2.5 md:px-3 h-10">
          <Languages className="h-4 w-4" />
          <span className="hidden sm:inline">{lang === 'en' ? 'हिंदी' : 'English'}</span>
          <span className="sm:hidden">{lang === 'en' ? 'हि' : 'EN'}</span>
        </Button>
        <Button variant="ghost" size="icon" className="relative h-10 w-10" onClick={() => navigate('/notices')} title={t('notices')}>
          <Bell className="h-5 w-5" />
          {unreadCount > 0 && (
            <span className="absolute top-1 right-1 flex h-4 min-w-4 px-1 items-center justify-center rounded-full bg-destructive text-[10px] font-bold text-destructive-foreground">
              {unreadCount > 9 ? '9+' : unreadCount}
            </span>
          )}
        </Button>
        <Button variant="ghost" size="icon" className="relative h-10 w-10" onClick={() => navigate('/my-profile')} title={t('my_profile')}>
          <UserCircle className="h-5 w-5" />
        </Button>
        <Button variant="ghost" size="icon" className="relative h-10 w-10 hidden sm:inline-flex" onClick={() => navigate('/change-password')} title={t('change_password')}>
          <KeyRound className="h-5 w-5" />
        </Button>
        <Button variant="ghost" size="icon" className="h-10 w-10" onClick={handleSignOut} title={t('sign_out')}>
