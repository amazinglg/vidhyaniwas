...
const ChangePassword = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) { toast.error(error.message); } else { toast.success(t('password_updated')); setPassword(''); setConfirm(''); }
    setLoading(false);
  };
  ...
  return (
    <div className="max-w-md mx-auto space-y-6">
      <div>
        <h1 className="text-3xl font-bold font-display text-foreground">{t('change_password')}</h1>
        <p className="text-muted-foreground mt-1">{t('update_password')}</p>
      </div>
      <Card className="p-6">
