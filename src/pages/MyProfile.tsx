...
const MyProfile = () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "maintenance_collections", filter: `resident_id=eq.${residentId}` },
        () => {
          fetchMaintenance();
        },
      )
  ...
  useEffect(() => {
    ...
    const fetchOwner = async () => {
      if (resident.owner_id) {
        const { data } = await supabase
          .from("residents")
          .select("name, mobile, house_no, lane_no")
          .eq("id", resident.owner_id)
          .maybeSingle();
        setHouseOwner(data);
      } else {
        ...
        const { data } = await supabase
          .from("residents")
          .select("name, mobile, house_no, lane_no")
          .eq("house_no", resident.house_no)
          .eq("lane_no", resident.lane_no)
  ...
  const openAddFamily = () => {
    ...
    setFamilyForm({ name: "", relation: "Other", age: "", occupation: "" });
    setFamilyDialog(true);
  };
  const openEditFamily = (fm: any) => {
  ...
  const openAddVehicle = () => {
    ...
    setVehicleForm({ vehicle_type: "Car", registration_no: "", make_model: "", color: "" });
  ...
  const handleSaveVehicle = async () => {
    if (!vehicleForm.registration_no || !residentId) {
      toast.error(t("please_fill_required"));
      return;
    }
    const payload = {
      ...
      registration_no: vehicleForm.registration_no,
      make_model: vehicleForm.make_model || null,
  ...
  const openAddTenant = () => {
    ...
    setTenantForm({ name: "", mobile: "" });
    setTenantDialog(true);
  };
  const openEditTenant = (tn: any) => {
    setEditingTenantId(tn.id);
    setTenantForm({ name: tn.name, mobile: tn.mobile });
    setTenantDialog(true);
  };
  const handleSaveTenant = async () => {
    if (!tenantForm.name || !tenantForm.mobile || !residentId || !resident) {
      toast.error(t("please_fill_required"));
      return;
    }
    if (!editingTenantId && tenants.length >= 1) {
      toast.error("Only one tenant per house is allowed");
      return;
    }
    const payload = {
      ...
      lane_no: resident.lane_no,
      resident_type: "tenant" as const,
      owner_id: residentId,
    };
    if (editingTenantId) {
  ...
  const handleDownloadReceipt = async (m: any) => {
    const { data: receipt } = await supabase
      ...
      .select("*")
      .eq("maintenance_collection_id", m.id)
      .maybeSingle();
    const r: any = receipt || {};
    downloadReceipt({
    ...
    <div className="space-y-4 max-w-3xl mx-auto">
          ...
          <div className="min-w-0 flex-1">
            ...
            {resident && (
              <p className="text-[11px] text-muted-foreground truncate">
                {t("house")} {resident.house_no} • {t("lane")} {resident.lane_no}
              </p>
            )}
          </div>
          {!editing && (
        ...
        {editing ? (
          <div className="grid gap-3 pb-3">
            ...
            <div className="flex gap-2">
              <Button size="sm" onClick={handleSave} className="h-8 text-xs gradient-warm text-primary-foreground">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 pb-3">
            <div className="flex items-center gap-2 p-2 rounded-lg bg-card border">
              ...
              <div className="min-w-0">
                <p className="text-[10px] text-muted-foreground leading-none">{t("mobile")}</p>
                <p className="text-xs font-medium truncate">{form.mobile || t("not_set")}</p>
              </div>
            </div>
            <div className="flex items-center gap-2 p-2 rounded-lg bg-card border">
              <Home className="h-3.5 w-3.5 text-primary shrink-0" />
              <div className="min-w-0">
                <p className="text-[10px] text-muted-foreground leading-none">{t("house_no")}</p>
                <p className="text-xs font-medium truncate">{resident?.house_no || t("not_set")}</p>
              </div>
            </div>
            {resident && (
              <div className="flex items-center gap-2 p-2 rounded-lg bg-card border">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] text-muted-foreground leading-none">{t("family_members")}</p>
                  <p className="text-xs font-medium truncate">{resident.family_members || 1}</p>
                </div>
              </div>
            )}
          </div>
        ...
        <Card className="px-5 py-2 bg-gradient-to-br from-amber-50/50 to-orange-50/50 dark:from-amber-950/20 dark:to-orange-950/20 border-amber-200 dark:border-amber-800">
          <div className="flex items-center gap-2 pt-3 pb-2">
            ...
            <h3 className="text-sm font-bold font-display uppercase tracking-wide">
              {lang === "hi" ? "मकान मालिक" : "House Owner"}
            </h3>
          </div>
          <div className="grid grid-cols-2 gap-2 pb-3">
            <div className="flex items-center gap-2 p-2 rounded-lg bg-card border">
              ...
              <div className="min-w-0">
                <p className="text-[10px] text-muted-foreground leading-none">{t("name")}</p>
                <p className="text-xs font-medium truncate">{houseOwner.name}</p>
              </div>
            </div>
            <div className="flex items-center gap-2 p-2 rounded-lg bg-card border">
              <Phone className="h-3.5 w-3.5 text-primary shrink-0" />
              <div className="min-w-0">
                <p className="text-[10px] text-muted-foreground leading-none">{t("mobile")}</p>
                <p className="text-xs font-medium truncate">{houseOwner.mobile}</p>
              </div>
            </div>
            <div className="flex items-center gap-2 p-2 rounded-lg bg-card border">
              <Home className="h-3.5 w-3.5 text-primary shrink-0" />
              <div className="min-w-0">
                <p className="text-[10px] text-muted-foreground leading-none">{t("house_no")}</p>
                <p className="text-xs font-medium truncate">{houseOwner.house_no}</p>
              </div>
            </div>
            <div className="flex items-center gap-2 p-2 rounded-lg bg-card border">
              <Home className="h-3.5 w-3.5 text-primary shrink-0" />
              <div className="min-w-0">
                <p className="text-[10px] text-muted-foreground leading-none">{t("lane")}</p>
                <p className="text-xs font-medium truncate">{houseOwner.lane_no}</p>
              </div>
            </div>
          </div>
        </Card>
      ...
      {residentId && isOwner && resident && (
        <LinkedMembersCard
          ...
          ownerHouseNo={resident.house_no}
          ownerLaneNo={resident.lane_no}
        />
      )}
      ...
      {residentId && isOwner && (
        <Card className="px-5 py-2">
          <SectionHeader
            ...
            action={
              tenants.length === 0 && (
                <Button size="sm" className="h-8 text-xs gradient-warm text-primary-foreground" onClick={openAddTenant}>
                ...
                {tenants.map((tn) => (
                  ...
                  >
                    ...
                    <div className="flex gap-0.5 shrink-0">
                      <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => openEditTenant(tn)}>
                        <Edit2 className="h-3.5 w-3.5" />
                      </Button>
                      <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => handleDeleteTenant(tn.id)}>
      ...
      {residentId && (
        <Card className="px-5 py-2">
          <SectionHeader
            ...
            action={
              canEditUnit && (
                <Button size="sm" className="h-8 text-xs gradient-warm text-primary-foreground" onClick={openAddFamily}>
                  <Plus className="h-3.5 w-3.5 mr-1" />
                  {t("add")}
                </Button>
              )
              ...
              <div className="space-y-1.5">
                {familyMembers.map((fm) => (
                  ...
                  >
                    ...
                    {canEditUnit && (
                      <div className="flex gap-0.5 shrink-0">
                        <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => openEditFamily(fm)}>
                          <Edit2 className="h-3.5 w-3.5" />
                        </Button>
                        <Button
                          variant="ghost"
          ...
          <SectionHeader
            ...
            action={
              canEditUnit && (
                <Button
                  size="sm"
                  className="h-8 text-xs gradient-warm text-primary-foreground"
              ...
              <div className="space-y-1.5">
                {vehicles.map((v) => (
                  ...
                  >
                    ...
                    {canEditUnit && (
                      <div className="flex gap-0.5 shrink-0">
                        <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => openEditVehicle(v)}>
                          <Edit2 className="h-3.5 w-3.5" />
                        </Button>
                        <Button
                          variant="ghost"
        ...
        <Card className="px-5 py-2">
          ...
          <div className="grid grid-cols-2 gap-2 mb-3">
            ...
            <div className="p-2.5 rounded-lg bg-orange-50 dark:bg-orange-950/30 border border-orange-200 dark:border-orange-800">
              <p className="text-[10px] text-orange-600 dark:text-orange-400 font-medium uppercase tracking-wide">
                {t("total_pending")}
              </p>
                      ...
                      return (
                        <TableRow key={m.id}>
                          ...
                          <TableCell className="text-orange-600 font-medium">
                            ₹{Number(m.due_amount || 0).toLocaleString()}
                          </TableCell>
                  ...
                  return (
                    <div key={m.id} className="p-2.5 rounded-lg border bg-muted/30 space-y-1.5">
                      <div className="flex items-center justify-between">
                        ...
                        <Badge variant={statusBadge[m.status] || "outline"} className="gap-1 text-[10px] h-5">
                          <StatusIcon className="h-3 w-3" />
                          {t(m.status)}
                        </Badge>
                      </div>
                      <div className="grid grid-cols-3 gap-2">
                        <div>
                          <p className="text-muted-foreground text-[10px] uppercase tracking-wide">{t("total")}</p>
                          <p className="text-xs font-medium">₹{Number(m.total_maintenance || 0).toLocaleString()}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground text-[10px] uppercase tracking-wide">{t("paid")}</p>
                          <p className="text-xs font-medium text-green-600">
                            ₹{Number(m.amount || 0).toLocaleString()}
                          </p>
                        </div>
                        <div>
                          <p className="text-muted-foreground text-[10px] uppercase tracking-wide">{t("due")}</p>
      ...
      <Dialog open={familyDialog} onOpenChange={setFamilyDialog}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>
              {editingFamilyId ? t("edit") : t("add")} {t("family_member_details")}
            </DialogTitle>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            ...
            <Button onClick={handleSaveFamily} className="w-full">
              {editingFamilyId ? t("update") : t("add")}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={vehicleDialog} onOpenChange={setVehicleDialog}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>
              {editingVehicleId ? t("edit") : t("add")} {t("vehicle")}
            </DialogTitle>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
            ...
            <div className="grid gap-2">
              ...
              <Input
                ...
                onChange={(e) => setVehicleForm({ ...vehicleForm, registration_no: e.target.value })}
                placeholder="e.g. UP14XX1234"
              />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
            ...
            <Button onClick={handleSaveVehicle} className="w-full">
              {editingVehicleId ? t("update") : t("add")}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={tenantDialog} onOpenChange={setTenantDialog}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editingTenantId ? t("edit_tenant") : t("add_tenant")}</DialogTitle>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label>{t("name")} *</Label>
            ...
            <Button onClick={handleSaveTenant} className="w-full gradient-warm text-primary-foreground">
              {editingTenantId ? t("update") : t("add_tenant")}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
