...
const Residents = () => {
  ...
  allResidents
    .filter((r: any) => r.resident_type === "tenant")
    .forEach((t: any) => {
      if (t.owner_id) {
  ...
  useEffect(() => {
    const channel = supabase
      .channel("residents-page-realtime")
  ...
  const uniqueLanes = Array.from(new Set(owners.map((r: any) => r.lane_no).filter(Boolean))).sort((a: any, b: any) =>
  ...
  const openEdit = (r: any) => {
    ...
    setForm({
      ...
      lane_no: r.lane_no,
      mobile: r.mobile,
      family_members: String(r.family_members || 1),
  ...
  const handleSave = async () => {
    if (!form.name || !form.house_no || !form.mobile) {
    ...
    if (form.resident_type === "member" || form.resident_type === "tenant") {
      const { data: ownersList } = await supabase
        .from("residents")
        .select("id")
        .eq("house_no", form.house_no)
        .eq("lane_no", form.lane_no)
  ...
  const [capDialog, setCapDialog] = useState<{ open: boolean; amount: number; projectedDue: number }>({
    open: false,
    amount: 0,
    projectedDue: 0,
  });
  ...
  const handleDownloadResidentReceipts = async (r: any) => {
    ...
    if (!receipts || receipts.length === 0) {
      toast.info(t("no_receipts"));
      return;
    }
    const rec: any = receipts[0];
    downloadReceipt({
      societyName: rec.society_name || "Shri Vidhya Niwas Society",
  ...
  return (
    <div className="space-y-4 md:space-y-6">
            ...
            {isAdmin && (
              <>
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setBulkAmountOpen(true)}
              ...
              <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                <DialogTrigger asChild>
                  <Button onClick={openAdd} size="sm" className="h-8 px-2.5 text-xs shrink-0">
                    <Plus className="h-3.5 w-3.5 mr-1" /> {t("add_resident")}
                  </Button>
                </DialogTrigger>
                <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
                  <DialogHeader>
                    <DialogTitle className="font-display">
                      {editingId ? t("edit_resident") : t("add_resident")}
                    </DialogTitle>
                  </DialogHeader>
                  <div className="grid gap-4 py-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="grid gap-2">
                        <Label>{t("lane_no")}</Label>
                        <Input value={form.lane_no} onChange={(e) => setForm({ ...form, lane_no: e.target.value })} />
                      </div>
                    </div>
                    <div className="grid gap-2">
                    ...
                    <Button onClick={handleSave} className="w-full mt-2">
                      {editingId ? t("update") : t("add_resident")}
                    </Button>
                  </div>
                </DialogContent>
              </Dialog>
          ...
          <Select value={laneFilter} onValueChange={setLaneFilter}>
            ...
            <SelectContent>
              <SelectItem value="all">
                {t("all")} {t("lane").toLowerCase()}s
              </SelectItem>
              {uniqueLanes.map((ln: any) => (
                <SelectItem key={String(ln)} value={String(ln)}>
                  {t("lane")} {ln}
                </SelectItem>
              ))}
            </SelectContent>
            ...
            <SectionCard key={r.id} className="py-3 space-y-2">
              <div className="flex items-center justify-between">
                <div>
                  {canViewDetails ? (
                    <button
                      className="text-primary hover:underline cursor-pointer font-semibold text-sm text-left"
              ...
              <div className="text-xs text-muted-foreground space-y-0.5">
                ...
                {isAdmin && (
                  <span className="flex items-center gap-1 text-foreground font-medium">
                    ...
                    {Number(r.maintenance_amount || 0).toLocaleString("en-IN")} / yr
                    <button onClick={() => openMaintAmount(r)} className="ml-1 text-primary">
              ...
              {!readOnly && (
                <div className="flex gap-1 pt-1 border-t flex-wrap">
                  ...
                  {isAdmin && (
                    <Button variant="ghost" size="sm" title="Move out" onClick={() => setMoveOutResident(r)}>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              <TableRow>
                <TableCell
                  colSpan={isAdmin ? (readOnly ? 7 : 8) : readOnly ? 6 : 7}
      ...
      <BulkUpdateAmountDialog open={bulkAmountOpen} onOpenChange={setBulkAmountOpen} residents={owners} />
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3 py-2">
            ...
            <div className="grid gap-2">
              ...
              />
              <p className="text-xs text-muted-foreground">
                This will create or update the pending due for {new Date().getFullYear()}. Already-paid entries will not
                be changed.
              </p>
            </div>
            <Button onClick={saveMaintAmount} className="w-full">
              Save
            </Button>
          </div>
        </DialogContent>
      ...
      <Dialog
        open={capDialog.open}
        onOpenChange={(o) => !o && setCapDialog({ open: false, amount: 0, projectedDue: 0 })}
      >
        <DialogContent className="max-w-sm">
          <DialogHeader>
          ...
          <div className="space-y-2 text-sm">
            <p>
              This change will make the resident's projected total due{" "}
              <strong>₹{capDialog.projectedDue.toLocaleString("en-IN")}</strong> for this financial year, breaching the
              ₹10,000 cap.
            </p>
            <p className="text-xs text-muted-foreground">
              Choose <strong>Cancel</strong> to abort, or <strong>Override & Save</strong> to apply anyway.
            </p>
          </div>
          <div className="flex justify-end gap-2 mt-4">
            <Button variant="outline" onClick={() => setCapDialog({ open: false, amount: 0, projectedDue: 0 })}>
              Cancel
            </Button>
            <Button
              onClick={() => doSaveMaintAmount(capDialog.amount)}
