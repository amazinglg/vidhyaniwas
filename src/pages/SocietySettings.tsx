...
import { Building2, Shield, Users, Settings, KeyRound, Edit2, Trash2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Master Administrator controls</p>
      </div>

      <Tabs defaultValue="society" className="w-full">
        <TabsList className="grid w-full grid-cols-3">
          <TabsTrigger value="society"><Building2 className="h-4 w-4 mr-2" />Society Info</TabsTrigger>
          <TabsTrigger value="users"><Users className="h-4 w-4 mr-2" />Manage Users</TabsTrigger>
          <TabsTrigger value="roles"><Shield className="h-4 w-4 mr-2" />Manage Roles</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="society" className="mt-6">
          <Card className="p-6 bg-gradient-to-br from-primary/5 to-accent/5 border-primary/20">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="p-3 rounded-lg bg-card border"><span className="text-muted-foreground">Total Houses:</span> <span className="font-medium">40+</span></div>
              <div className="p-3 rounded-lg bg-card border"><span className="text-muted-foreground">Lanes:</span> <span className="font-medium">4</span></div>
              <div className="p-3 rounded-lg bg-card border"><span className="text-muted-foreground">Monthly Maintenance:</span> <span className="font-medium">₹3,000 per house</span></div>
              <div className="p-3 rounded-lg bg-card border"><span className="text-muted-foreground">Master Admin:</span> <span className="font-medium">Labhansh Garg</span></div>
            </div>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="roles" className="mt-6 space-y-4">
          <Card className="p-4">
            <h3 className="text-lg font-semibold font-display mb-4 flex items-center gap-2"><Shield className="h-5 w-5 text-primary" /> User Roles & Password Management</h3>
            <Table>
              <TableHeader>
                <TableRow>
          ...
          <Card className="p-4">
            <h3 className="text-lg font-semibold font-display mb-3">Available Roles</h3>
      ...
      <Dialog open={!!editResident} onOpenChange={() => setEditResident(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle className="font-display">Edit Resident</DialogTitle></DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2"><Label>Full Name</Label><Input value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} /></div>
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2"><Label>House No.</Label><Input value={editForm.house_no} onChange={(e) => setEditForm({ ...editForm, house_no: e.target.value })} /></div>
              <div className="grid gap-2"><Label>Lane No.</Label><Input value={editForm.lane_no} onChange={(e) => setEditForm({ ...editForm, lane_no: e.target.value })} /></div>
            </div>
            <div className="grid gap-2"><Label>Mobile</Label><Input value={editForm.mobile} onChange={(e) => setEditForm({ ...editForm, mobile: e.target.value })} /></div>
            ...
            <Button onClick={handleSaveResident} className="w-full gradient-warm text-primary-foreground">Save Changes</Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!roleDialog} onOpenChange={() => setRoleDialog(null)}>
        <DialogContent className="max-w-sm">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Select value={newRole} onValueChange={(v) => setNewRole(v as AppRole)}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="president">Society President</SelectItem>
            ...
            <Button onClick={handleChangeRole} className="w-full gradient-warm text-primary-foreground">Update Role</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
